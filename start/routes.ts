...
Route.get('/', async ({ view, auth, request, response }) => {
  ...
  if (auth.user) {
    if (auth.user.role === 'student') {
      return response.redirect('/student-information/' + auth.user.user_id)
    } else {
...
Route.get('/dashboard', 'DashboardsController.showDashboard').middleware('login').middleware('role')
...
Route.group(() => {
  Route.get('/', 'StepsController.showStudentUser').middleware('role')
  Route.get('/:id', 'StepsController.showStudentUserById')
...
Route.get('/register-request', 'StepsController.showStudentUser').middleware('login')

Route.get('/course-info', 'StepsController.showStudentUser').middleware('login').middleware('role')
...
Route.get('/steps', 'StepsController.showStudentUser').middleware('login').middleware('role')

Route.get('/step/edit', 'StepsController.showStudentUser').middleware('login').middleware('role')
...
Route.group(() => {
  ...
  Route.get('/create', 'PostsController.showCreate').middleware('role')

  Route.get('/edit/:id', 'PostsController.showEdit').middleware('role')
...
Route.group(() => {
  ...
  Route.get('/logout', 'UsersController.logout').as('auth.logout').middleware('login')
  Route.group(() => {
    ...
    Route.patch(
      '/student/regis/approve',
      'CoursesInfoController.updateStudentUserApprove'
    ).middleware('role')
    Route.delete('/student/:id', 'UsersController.deleteStudentUser').middleware('role') //
    Route.patch('/courseInfo', 'CoursesInfoController.updateCourseInformation').middleware('role')
    Route.patch('/courseInfoUs', 'CoursesInfoController.updateUsersCourseInformation').middleware(
      'role'
    )
    Route.patch('/advisor/st', 'CoursesInfoController.updateAdvisorHasStudent').middleware('role')
    Route.get('/advisorUser', 'UsersController.showAdvisorUser').middleware('role')
    Route.get('/staffUser', 'UsersController.showStaffUser').middleware('role')
    Route.get('/staffCYear', 'UsersController.getStaffUserCuurentYear').middleware('role')
    Route.get('/advisorCYear', 'UsersController.getAdvisorUserCuurentYear').middleware('role')
    Route.get('/studentCYear', 'UsersController.getStudentUserCuurentYear').middleware('role')
    Route.get('/studentByAdv/:id', 'UsersController.getStudentUserByAdvisor').middleware('role')
    Route.delete('/delUserAc/:id', 'UsersController.delUsersInAcademicYear').middleware('role')
    Route.delete('/delUserAdv/:id', 'UsersController.delUsersFromAdvisor').middleware('role')
  ...
  Route.group(() => {
    Route.post('/', 'PostsController.store').middleware('role')
    Route.patch('/:id', 'PostsController.update').middleware('role')
    Route.delete('/:id', 'PostsController.remove').middleware('role')
  ...
  Route.get('/file', 'FilesController.showAllFile').middleware('login').middleware('role')
  Route.patch('/course/:id', 'CoursesInfoController.completeCourse')
    .middleware('login')
    .middleware('role')

  Route.group(() => {
    Route.post('/', 'FilesController.store').middleware('role')
    Route.post('/steps', 'FilesController.storeDirect')
    Route.get('/:fileId', 'FilesController.downloadFile')
    Route.delete('/:fileId', 'FilesController.deleteFileDirect').middleware('role')
  })
    .middleware('login')
    // .middleware('login')
    .prefix('/file')
